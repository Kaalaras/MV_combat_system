...
def _build_signature_catalog() -> dict[tuple[int, int | None, int], list[str]]:
    ...
    Each entry maps the signature generated by :func:`_descriptor_signature`
    back to the descriptor name (or names in the case of combined terrains)
    needed to recreate that descriptor when serialising grid data.
    """
    ...
    for r in range(2, len(names) + 1):
        for combo in combinations(names, r):
            ...
            # The catalog size grows combinatorially (O(2^n)) with the number of
            # base descriptors.  The terrain catalog is intentionally small, so
            # building the full lookup up-front remains practical and avoids
            # repeated combination work during conversions.
