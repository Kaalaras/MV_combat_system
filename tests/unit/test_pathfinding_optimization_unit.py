...
class TestPathFindingOptimizationUnit(unittest.TestCase):
    ...
    def test_portal_on_map_edge(self):
        """Portals should not be created outside the map bounds, even at the edge."""
        terrain = DummyTerrain(10, 10)
        pf = OptimizedPathfinding(terrain)
    ...
    def test_single_tile_region(self):
        """A 1x1 region should not create portals or crash."""
    ...
    def test_disconnected_regions(self):
        """No portals should be created between regions separated by a wall."""
        ...
        for region in pf.regions.values():
            for p in region['portals']:
                self.assertNotEqual(p[0], 5)  # No portal on the wall

    def test_portal_overlap(self):
        """Single-tile opening between regions should create only one portal."""
        walls = [(5, y) for y in range(10) if y != 5]
        terrain = DummyTerrain(10, 10, walls=walls)
        pf = OptimizedPathfinding(terrain)
        pf.min_region_size = 3
        pf._create_hierarchical_waypoints((0, 0, 10, 10))
        # There should be exactly one portal at (5,5) or (4,5)
        found = False
        for region in pf.regions.values():
            for p in region['portals']:
                if (p == (5, 5) or p == (4, 5)):
    ...
    def test_leaf_region_lookup_out_of_bounds(self):
        """Looking up a position outside the map should return None."""
        terrain = DummyTerrain(10, 10)
        pf = OptimizedPathfinding(terrain)
        pf.min_region_size = 3
        pf._create_hierarchical_waypoints((0, 0, 10, 10))
