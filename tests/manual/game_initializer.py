...
def _deprecated_initialize_game(entity_specs, grid_size, max_rounds=200, map_dir="battle_maps"):
    ...
    This function automates the entire setup process:
    1. Creates and wires the core components (GameState, EventBus, Terrain).
    2. Populates the world with entities based on the provided specs.
    3. Sets up alliances between entities.
    4. Initializes and registers all game systems (Movement, Actions, AI, Turns).
    5. Prepares the game for start by running the PreparationManager.
    6. Sets up the GameSystem controller, ready to run the simulation.

    Args:
        entity_specs: A list of EntitySpec objects defining the entities to create.
        grid_size: The width and height of the game grid.
        max_rounds: The maximum number of rounds for the simulation.
        map_dir: The directory to save battle map images to.
    ...
    for idx, spec in enumerate(entity_specs):
        ...
        if terrain.add_entity(entity_id, spec.pos[0], spec.pos[1]):
            ...
            print(f"Warning: Could not place entity {entity_id} at {spec.pos}. Position may be occupied or invalid.")
            # If placement fails, we should probably remove it from the game state as well.
    ...
    # It now runs optimizers and pre-calculates caches on a populated map.
